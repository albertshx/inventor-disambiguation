...
/**
  ...
  */
class EvaluateCoreference[P <: Comparable[P], C](algName: String,
                                                 predicted: Iterable[(P, C)],
                                                 gold: Iterable[(P, C)]) {
  ...
  def evaluate = {
    ...
    assert(
      predPointsUniq == goldPointsUniq,
      s"The two clustering results must be defined on the same set of points. There are ${predPointsUniq.size} predicted points and ${goldPointsUniq.size} gold points. There are ${predPointsUniq.diff(goldPointsUniq).size} points in the predicted set that do not appear in the gold set. There are ${goldPointsUniq.diff(predPointsUniq).size} points in the gold set that do not appear in the predicted set."
    )
    assert(
      predPoints.size == predPointsUniq.size,
      "The predicted clustering contains at least one point multiple times.")
    assert(goldPoints.size == goldPointsUniq.size,
           "The gold clustering contains at least one point multiple times.")
    ...
    val mdString = Iterable(algName,
                            ...
                            truePositives.size,
                            falsePositives.size,
                            falseNegatives.size).mkString("| ", " | ", " |")
    Map(
      "Num. Pred Entities" -> numEntities,
      "Num. Gold Entities" -> numGoldEntities,
      "recall" -> recall,
      "splitting" -> splitting,
      "lumping" -> lumping,
      "precision" -> precision,
      "f1" -> f1,
      "Num. True Positives" -> truePositives.size,
      "Num. False Negatives" -> falseNegatives.size,
      "Num. False Positives" -> falsePositives.size,
      "summary" -> mdString
    )
  }

  def formattedString(map: Map[String, Any]) =
    (Iterable("Clustering Results:\n") ++ map.map(f => f._1 + ": " + f._2))
      .mkString("\n")

  def collectCoreferencePairs(clustering: Iterable[(P, C)]) = {
    val byClusterID =
      clustering.groupBy(_._2).mapValues(_.map(_._1).toIndexedSeq.sorted)
    val coreferencePairs = new ArrayBuffer[(P, P)](
      byClusterID.map(ps => ps._2.size * ps._2.size).sum)
    byClusterID.foreach {
  ...
  def pairwiseHTMLReport(pairs: Iterable[(P, P)],
                         lookup: P => PatentsViewRecord,
                         title: String) = {
  ...
  def errorAnalysis(lookup: P => PatentsViewRecord) = {
    ...
    val FP_sameFirstAndLastNameSpelling = falsePositives.count { pair =>
    ...
    val FN_sameFirstAndLastNameSpelling = falseNegatives.count { pair =>
      val m1 = lookup(pair._1)
      val m2 = lookup(pair._2)
      (m1, m2) match {
        case ((inv1: InventorMention, inv2: InventorMention)) =>
    ...
    val FN_sameFullNameSpelling = falseNegatives.count { pair =>
      val m1 = lookup(pair._1)
      val m2 = lookup(pair._2)
      (m1, m2) match {
        case ((inv1: InventorMention, inv2: InventorMention)) =>
    ...
    val FN_sameLastDiffFirst = falseNegatives.count { pair =>
      val m1 = lookup(pair._1)
      val m2 = lookup(pair._2)
      (m1, m2) match {
        case ((inv1: InventorMention, inv2: InventorMention)) =>
          inv1.self.value.sameLastName(inv2.self.value) && !inv1.self.value
            .sameFirstName(inv2.self.value)
        case _ =>
          false
      }
    }
    ...
    val FN_differentLastName = falseNegatives.count { pair =>
      val m1 = lookup(pair._1)
      val m2 = lookup(pair._2)
      (m1, m2) match {
        case ((inv1: InventorMention, inv2: InventorMention)) =>
    ...
    Map(
      "False Positives: sameFirstAndLastNameSpelling" -> FP_sameFirstAndLastNameSpelling,
...
object DebugHelpers {
  ...
  def falsePositivesByCanopy(falsePositivePairs: Iterable[(String, String)],
                             mentions: Iterable[Mention[InventorVars]]) = {
    val byID = mentions.groupBy(_.uniqueId).mapValues(_.head)
    val falsePositiveMentionPairs =
      falsePositivePairs.map(f => (byID(f._1), byID(f._2)))
    val byCanopy = falsePositiveMentionPairs
      .groupBy(_._1.variables.canopy)
      .mapValues(v => v.map(f => (f._1.uniqueId, f._2.uniqueId)))
    byCanopy
  }

  def writeFalsePositivesByCanopy(
      file: File,
      map: Map[String, Iterable[(String, String)]]) = {
    ...
    map.foreach {
      case (canopy, mentions) =>
        ...
        pw.print("\t")
        pw.print(
          mentions.flatMap(f => Iterable(f._1, f._2)).toSet.mkString(","))
        pw.println()
        pw.flush()
    }
    pw.close()
  ...
  def pairwisePlainTextReportByCanopy(
      pairs: Iterable[(String, String)],
      mentions: Iterable[Mention[InventorVars]]) = {
    val byID = mentions.groupBy(_.uniqueId).mapValues(_.head)
    val MentionPairs = pairs.map(f => (byID(f._1), byID(f._2)))
    val byCanopy = MentionPairs
      .groupBy(_._1.variables.canopy)
      .mapValues(v =>
        v.map(f =>
          (f._1.variables.provenance.get.self.value.debugString(),
           f._2.variables.provenance.get.self.value.debugString())))
    byCanopy
  }

  def writePairwisePlainTextReportByCanopy(
      file: File,
      map: Map[String, Iterable[(String, String)]]) = {
    val pw = new PrintWriter(file)
    map.foreach {
