...
import edu.umass.cs.iesl.inventor_disambiguation.db.{
  InMemoryPrimaryKeyDatastore,
  InventorMentionDB,
  MongoDBOpts
}
...
/**
  ...
  */
trait RunParallelOpts
    extends MongoDBOpts
    with InventorModelOptions
    with InMemoryHashMapKeystoreOpts {
  val corefTaskFile = new CmdOption[String](
    "coref-task-file",
    "The file containing the coref tasks",
    true)
  val codec =
    new CmdOption[String]("codec", "UTF-8", "STRING", "The encoding to use.")
  val outputDir =
    new CmdOption[String]("output-dir", "Where to write the output", true)
  val numThreads =
    new CmdOption[Int]("num-threads", 20, "INT", "Number of threads to use")
  val convertToOneBasedOutputIds = new CmdOption[Boolean](
    "convert-to-one-based-output-ids",
    true,
    "BOOLEAN",
    "whether to convert the output inventor ids to a one based format")
}

trait RunParallelInMemoryOpts extends RunParallelOpts {
  val inputJson = new CmdOption[String](
    "input-json",
    "input file with one JSON InventorMention per line.")
}
...
object RunParallelSingleCanopyCoreference {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val allWork = LoadCorefTasks
      .load(new File(opts.corefTaskFile.value), opts.codec.value)
      .toIterable
    ...
    val keystore = InMemoryKeystore.fromFile(new File(opts.keystorePath.value),
                                             opts.keystoreDim.value,
                                             opts.keystoreDelim.value,
                                             opts.codec.value)

    new File(opts.outputDir.value).mkdirs()
    val parCoref = new SingleCanopyParallelHierarchicalCoref(
      allWork,
      ds,
      opts,
      keystore,
      new File(opts.outputDir.value),
      opts.codec.value,
      opts.convertToOneBasedOutputIds.value)
...
object RunParallelMultiCanopyCoreference {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parCoref = new MultiCanopyParallelHierarchicalCoref(
      allWork,
      ds,
      opts,
      keystore,
...
object RunParallelMultiCanopyCoreferenceInMemory {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val inventorMentions = LoadJSONInventorMentions.load(
      new File(opts.inputJson.value),
      opts.codec.value)

    // Create a hashmap of the inventor mentions by id
    val ds = InMemoryPrimaryKeyDatastore.fromIterator(
      inventorMentions.map(im => (im.uuid.value, im)))
