...
class InventorMention extends PatentsViewRecord {
  ...
  val rawName =
    new CubbieSlot[PersonNameRecord]("rawName", () => new PersonNameRecord())

  val patent = new CubbieSlot[Patent]("patent", () => new Patent())
  val coInventors =
    new CubbieListSlot[Inventor]("coInventors", () => new Inventor())
  ...
  val applications =
    new CubbieListSlot[Application]("applications", () => new Application())

  val assignees =
    new CubbieListSlot[Assignee]("assignees", () => new Assignee())
  ...
  val usPatentCitations = new CubbieListSlot[USPatentCitation](
    "uspatentcitations",
    () => new USPatentCitation())
...
object InventorMention {
  ...
  def fromDatastores(
      ...
      inventorDB: Datastore[String, Inventor]): InventorMention = {

    val patentID = self.patentID.value
    val maybePatent = patentDB.get(patentID)
    if (maybePatent.isEmpty)
      println(s"[${this.getClass.getSimpleName}] WARNING:  We must have a patent for the inventor " +
        s"${self.uuid.value}, ${self.nameFirst.value}, ${self.nameLast.value} with patent ${self.patentID.value}")
    val mention = new InventorMention()
    mention.self.set(self)

    // Take the raw name split it up into middle and suffixes
    mention.rawName.set(
      new PersonNameRecord(self.nameFirst.opt,
                           self.nameMiddles.opt,
                           self.nameLast.opt,
                           self.nameSuffixes.opt))
    ...
    val coInventors = inventorDB
      .get(patentID)
      .filter(_.inventorID.value != self.inventorID.value)
  ...
  def fromDatastores(self: Inventor,
    ...
    mention.self.value.rawLocationID.opt.foreach { locId =>
      mention.self.value.location.set(locationDB.get(locId).headOption)
    }

    // Set the co-inventor's locations:
    mention.coInventors.value.foreach(inv =>
      inv.rawLocationID.opt.foreach { locId =>
