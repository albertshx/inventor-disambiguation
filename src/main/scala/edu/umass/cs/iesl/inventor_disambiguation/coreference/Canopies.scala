...
object Canopies {
  ...
  /**
    ...
    * @return the canopy
    */
  def lastNameAndFirstNCharactersOfFirst(inventor: Inventor,
                                         N: Int,
                                         caseSensitive: Boolean) = {
    val nameParts = Some("LAST_") ++ inventor.nameLast.opt.map(f =>
      if (caseSensitive) f else f.toLowerCase) ++ Some("_FIRST_") ++ inventor.nameFirst.opt
      .map(f => if (caseSensitive) f.slice(0, N) else f.toLowerCase.slice(0, N))
    nameParts.mkString("")
  }
  ...
  /**
    ...
    * @return the canopy
    */
  def lastNameAndFirstCharCaseInsensitive(inventor: Inventor) =
    lastNameAndFirstNCharactersOfFirst(inventor, 1, caseSensitive = false)
  ...
  /**
    ...
    * @return the canopy
    */
  def lastNameAndFirstThreeCharsCaseInsensitive(inventor: Inventor) =
    lastNameAndFirstNCharactersOfFirst(inventor, 3, caseSensitive = false)
  ...
  def firstAndLastName(inventor: Inventor, caseSensitive: Boolean) = {
    val nameParts = Some("LAST_") ++ inventor.nameLast.opt.map(f =>
      if (caseSensitive) f else f.toLowerCase) ++ Some("_FIRST_") ++ inventor.nameFirst.opt
      .map(f => if (caseSensitive) f else f.toLowerCase)
    nameParts.mkString("")
  }
  ...
  /**
    ...
    * @return the canopy
    */
  def firstAndLastNameCaseInsensitive(inventor: Inventor) =
    firstAndLastName(inventor, caseSensitive = false)
  ...
  def fullName(inventor: Inventor, caseSensitive: Boolean) = {
    val nameParts = Some("LAST_") ++ inventor.nameLast.opt.map(f =>
      if (caseSensitive) f else f.toLowerCase) ++ Some("_FIRST_") ++ inventor.nameFirst.opt
      .map(f => if (caseSensitive) f else f.toLowerCase) ++ Some("_MIDDLE_") ++ inventor.nameMiddles.opt
      .map(
        f =>
          if (caseSensitive) f.mkString(" ")
          else
            f.mkString(" ")
              .toLowerCase
              .trimBegEnd()) ++ Some("_SUFFIXES_") ++ inventor.nameSuffixes.opt
      .map(
        f =>
          if (caseSensitive) f.mkString(" ")
          else f.mkString(" ").toLowerCase.trimBegEnd())
    nameParts.mkString("")
  }
